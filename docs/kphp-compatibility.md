...
## PHP 7.4 в образе vkcom/kphp — это НЕ ваш PHP

Образ `vkcom/kphp` (Ubuntu 20.04 focal) поставляется с PHP 7.4 + `php7.4-vkext`.  
**Это только компилятор/тулинг** — KPHP сам парсит PHP-код и компилирует его в C++.  
Выходной бинарник **не зависит от PHP вообще**.

| | Версия | Назначение |
|---|---|---|
| Ваш код (src/) | PHP 8.1+ синтаксис | Runtime на shared hosting |
| `vkcom/kphp` образ | PHP 7.4 | Компилятор (детали реализации) |
| Выходной бинарник | — | Самодостаточный C++-бинарник |

**Следствие:** в `src/` можно использовать синтаксис PHP 8.1 (`readonly`, `enum`, `fibers` — см. ограничения ниже). KPHP поддерживает подмножество PHP 8.x.

---

## php-vkext — KPHP-стабы для разработки

`php-vkext` — это PHP-расширение, которое предоставляет **KPHP-специфичные функции** (`instance_cast()`, `fork()`, `wait()` и др.) как PHP-стабы. Оно нужно только для:

- IDE-интеграции (автодополнение KPHP-функций)
- Статического анализа PHPStan/Psalm с KPHP-профилем
- Запуска кода через обычный PHP (с KPHP-функциями в виде заглушек)

**Доступные версии vkext:** 7.4, 8.0, **8.1**, 8.2 — из репозитория `artifactory-external.vkpartner.ru`.

```bash
# Установка для PHP 8.1 (Ubuntu 20.04 focal)
...
apt-get update && apt-get install php8.1-vkext
```

> В `Dockerfile.check` `php8.1-vkext` устанавливается в стадию `kphp-build` для dev tooling. Сам компилятор `kphp` его **не требует**.
